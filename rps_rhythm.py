...
HOLD_TIME = 0.8           # 같은 제스처를 N초 이상 유지해야 확정 (시간 기반)
...
class GameState:
    ...
    def reset(self):
        ...
        # ── 리듬 모드 전용 ──
        rhythm_mode.reset_rhythm_fields(self)
    ...
    try:
        while True:
            ...
            if game.state == game.TITLE:
                ...
                rhythm_mode.draw_stage_select_screen(screen, game)
                ...
                rhythm_mode.rhythm_play_tick(screen, frame, game, now, stable_g,
                                             draw_gesture_icon, HEART_CACHE, _cache_heart)
                ...
                rhythm_mode.draw_rhythm_hud(screen, game, HEART_CACHE, _cache_heart)
                rhythm_mode.draw_rhythm_stage_clear(screen, game)
            ...
            if key_ch == 'q':
                ...
                if game.state == game.TITLE:
                    ...
                    game.state = game.COUNTDOWN
                    game.countdown_start = time.time()
                    print(f"[Rhythm {rhythm_mode.RHYTHM_DIFFICULTY[game.rhythm_difficulty]['name']}] [Stage 1]")
                elif game.state == game.STAGE_CLEAR:
                    game.stage_idx += 1
                    stages_len = rhythm_mode.rhythm_stages_count(game) if game.mode == 4 else len(STAGES)
                    if game.stage_idx >= stages_len:
